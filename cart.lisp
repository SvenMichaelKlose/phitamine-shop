...
(defun cart-remove ()
  (cart-add-undo)
  (= (cart-current) (aremove (request :id) (cart-current) :test #'string==))
  (request-action-redirect "cart"))
...
(defun cart ()
  (= *navi-template* (? (has-cart?)
                        #'tpl-cart
                        #'tpl-cart-empty)))

(define-navi-action cart)
(define-navi-action cart-add)
(define-navi-action cart-remove)
(define-navi-action cart-undo)
(define-navi-action cart-redo)
